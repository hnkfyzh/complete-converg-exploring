...
class GridEnv(gym.Env):
    def __init__(self, resolution, num_agents, max_steps,
        ...
        # self.pub_path_a = rospy.Publisher('astar_path_a', Float64MultiArray, queue_size=10)
        # self.pub_path_b = rospy.Publisher('astar_path_b', Float64MultiArray, queue_size=10)
        # self.pub_path_c = rospy.Publisher('astar_path_c', Float64MultiArray, queue_size=10)
    ...
    def reset_for_traditional(self):
        ...
        #图像处理操作
        map_img = np.array(map_img) #最原始的图像
        map_img = np.rot90(map_img,3)
        # kernel = np.ones((1, 2), np.uint8)
        # kernel_2 = np.ones((6, 6), np.uint8)
        # scaled_map = cv2.dilate(map_img, np.ones((6, 6), np.uint8), 1)
        scaled_map = cv2.erode(map_img, np.ones((6,6), np.uint8), 1)
        # scaled_map = cv2.dilate(scaled_map, np.ones((6, 6), np.uint8), 1)
        rows, cols = map_img.shape

        # 创建一个新图像，初始化为白色
        # scaled_map = np.ones((rows, cols), dtype=np.uint8) * 255

        # # 遍历每个像素
        # for i in range(rows):
        #     for j in range(cols):
        #         if map_img[i, j] == 0:  # 如果当前像素是黑色
        #             # 将3x3区域变为黑色
        #             for m in range(max(0, i - 4), min(rows, i + 5)):
        #                 for n in range(max(0, j - 4), min(cols, j + 5)):
        #                     scaled_map[m, n] = 0

        # scaled_map = scaled_map[:int(scaled_map.shape[0] *4 / 5), :]  # 图片裁剪
        # scaled_map = scaled_map[:, :2*int(scaled_map.shape[1] / 3)]
        scaled_map = scaled_map[int(scaled_map.shape[0]  / 5):, :]
        ...
        scaled_map_uint8 = scaled_map.astype(np.uint8)
        scaled_map_uint8 = cv2.resize(scaled_map_uint8,(1886,1331),interpolation=cv2.INTER_LINEAR)
        def on_EVENT_LBUTTONDOWN(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                self.agent_pos.append([int(b[-1]*self.width/1331), int(a[-1]*self.height/1886)])
    ...
    def get_goal_for_cost(self):
        ...
        while self.finish_exploration_step != [True,True,True]: #直到全部探索完
        ...
        # for e in range(self.num_agents):
            ...
            if self.finish_exploration[e] == False:
                ...
                while(not find_suitable_point):#一个循环，使机器人都找到合适的目标点，否则就finish
                    if Dis2Frs != []:
                        ...
                        elif global_plan != 0 and goal[0] != self.agent_pos[e][0] and goal[1] != self.agent_pos[e][1]:
                            # 目标点可以到达
    ...
    def step_for_cost(self):
        ...
        for i in range(self.num_agents):
            if self.finish_exploration == [True,True,True]:
                ...
                path_msg_a = Float64MultiArray()
                path_msg_b = Float64MultiArray()
                path_msg_c = Float64MultiArray()
                path_msg_a.data = robot_0_path
                path_msg_b.data = robot_1_path
                path_msg_c.data = robot_2_path
                ...
                i = 0
                while i<5:
                    self.pub_path_a.publish(path_msg_a)
                    self.pub_path_b.publish(path_msg_b)
                    self.pub_path_c.publish(path_msg_c)
                    self.rate.sleep()
                    i=i+1
                break
...
def use_cost_method_to_explore(agent_num, map_name):
    env = GridEnv(0.1,  agent_num, 1000, map_name,
